...
#checking card uid with database
def chck(ID):
...
def log_in(msg):
    ...
    if record:
        ...
        values=(msg[1], msg[3], msg[2], msg[4], msg[5])
...
#adding new card to database (admin control, card uid received from the reader)
def ad(msg):
    uid=msg[2]
...
#deleting card from the database(admin conrtol, card to be deleted given as input)
def dlt():
...
#always listening to the channel until keyboard interrupt
while True:
    try:
